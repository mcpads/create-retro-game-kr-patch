...
| 번역 | `references/strategy/translation-workflow.md` | 번역 작업·에이전트 배치, 맥락 확정, 승인 용어·문체, 보호 제약과 고위험 의미의 적격 판정 |
...
새 판단을 시작하거나 관측 증상이 바뀌면 `references/tips/README.md`에서 판단 영역과 `Read when`을 대조하고, 맞는 사례의 `Transfer limit`을 현재 대상에서 검증한다. 완료는 사례가 아니라 `Related criteria`가 가리키는 문서로 판정한다.

## 판단 흐름

- 선언한 완성 범위와 반드시 성립해야 할 조건을 먼저 확인한다.
- 실패하면 완성이 불가능하거나 설계를 크게 바꾸는 미확정 조건을 먼저 판정하고, 같은 조건·선행 상태·증명 범위를 보존하는 방법 사이에서 비용을 비교한다.
...
- **자유 문장의 초벌은 현재 에이전트나 같은 기준선·맥락의 서브에이전트가 맡는다.** 별도 모델·에이전트는 실제 대상의 대표 표본을 사람이 평가해 맡긴 범위에서만 쓴다. 분량·속도·비용은 검증되지 않은 번역자를 쓰는 근거가 아니다. 상세 조건은 `references/strategy/translation-workflow.md` §3.1을 따른다.
- **자유 문장의 최종 판정은 사람이 한다.** 자동 검사와 언어 휴리스틱은 후보와 영향 범위만 만들며 번역·검토를 대신하지 않는다. 자동 판정은 사람이 적용 단위와 허용값을 확정한 규칙, 보호 정보와 실제 소비 제약처럼 참·거짓이 정해진 위반에 한정한다. 선언한 번역 범위의 사람 검수는 릴리스 후보의 조건이며, 미완료 단위는 원문으로 두고 개발을 계속한다. 상세 경계는 `references/strategy/translation-workflow.md` §5를 따른다.
...
- **작업 단위는 완성 조건을 보존한다.** 대표 단위에는 함께 성립해야 할 경계와 확인된 제약 중 가장 까다로운 사례를 포함한다. 진단을 더 작게 나눴다면 결과를 원래 완성 조건으로 되돌려 판정한다.
