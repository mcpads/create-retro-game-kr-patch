...
서로 독립된 경계는 병렬로 조사할 수 있다. 새 증거가 기존 판단을 뒤집으면 영향받는 판정만 취소해 원인 경계로 돌아가고, 나머지 증거는 유지한다.

- 모집단·경계·소비 규칙이 확정되지 않은 텍스트는 번역·재삽입 완료로 승격하지 않는다.
- 대표 PoC는 모집단이 미확정이어도 수행할 수 있지만, 번역 작업을 전체 배포 범위로 확대하거나 전체 코퍼스 수요를 확정하기 전에 `references/strategy/text-extraction.md` §1.5의 분량 조사를 통과한다.
- 되돌리기 비싼 작업의 전제가 동등한 증거로 확인되지 않았다면 해당 PoC로 전제를 먼저 검증한다.
...
| 판단 영역 | 문서 | 내용 |
|------|------|------|
| 초기 조사 | `references/strategy/initial-survey.md` | 실제 의존 경계, 초기 분량·미확정 모집단, 확정 리비전 사양과 가장 싼 판정 증거 선택 |
...
tips 전체의 열람은 선택적이지만 색인 대조는 생략하지 않는다. strategy가 현재 판단 영역과 발동 조건을 식별하면 `references/tips/README.md`의 짧은 색인을 보고, 일치할 때만 가장 직접 맞는 사례 앵커를 읽는다. 독립된 발동 조건이 여러 개면 조건별로 고른다. 관측 플랫폼과 범위는 증거의 출처와 전이 한계이며 선택 필수조건이 아니다. `references/tips/general.md`도 통독하지 않는다. 사례는 조사 후보나 개입 가설을 제안할 뿐 현재 게임의 구조·원인·해법을 증명하지 않으며, 검증과 완료 조건은 strategy의 판단 기준을 따른다.
...
5. **판정 가능한 조사** — `references/strategy/initial-survey.md`에 따라 가장 싼 증거 앵커에서 실제 의존 경계를 조사하고, 결론과 남은 가설을 기록한다.
