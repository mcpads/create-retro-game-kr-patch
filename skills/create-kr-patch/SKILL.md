...
- **Preserve completion conditions in every work unit.** A representative unit includes all boundaries that must hold together and the hardest confirmed constraint. If diagnosis uses smaller units, return the result to the original completion condition before accepting it. See `references/strategy/poc.md` §4.
