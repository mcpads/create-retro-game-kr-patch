...
- **원본에서 목표 이미지를 만드는 모든 변경은 Expected Write로 사전 등록되어야 한다.** 불변 원본을 기준으로 허용 범위·소유자·최종 바이트를 적용 전에 검증하며, 후처리를 포함한 우회 쓰기·충돌·보호 범위 침범·추적되지 않은 diff를 빌드 실패로 처리한다.
- **기계어 생성·이동은 선언한 ISA 프로필 전체에서 검산한다.** 고정된 짧은 기대 바이트를 제외하고 패치에 필요한 명령만 구현한 부분 집합으로 생성·이동·제어 흐름의 완전성을 증명하지 않는다. 상세 조건은 `references/conventions/project-conventions.md` §2.3을 따른다.
...
- **필요한 PoC 게이트를 통과하기 전에 되돌리기 비싼 작업에 착수하지 않는다.** 가시성·수용량처럼 후속 작업을 뒤집을 위험이 동등한 증거로 해소되지 않았다면, 전체 배포 범위의 사람 검수·최종 다듬기, 전체 폰트 생성, 엔진 본 패치 같은 작업은 해당 게이트가 통과될 때까지 보류한다. 초벌 코퍼스는 이 게이트의 입력일 뿐 검수 완료 번역이 아니다. PoC는 목표(0원칙)를 가장 크게 위협하는 블로커를 직접 검증하며, 더 싼 저위험 실험으로 이를 대체하지 않는다.
- **사례 수치를 새 게임에 그대로 가정하지 않는다.** 글리프 슬롯 수, 빈 공간 크기, 포인터 폭 같은 수치는 선행 사례가 아니라 이 게임에서 재실측한 값을 쓴다. 특히 글리프 예산은 고정값이 아니라 병목 자원 산정 공식의 출력이다(`references/strategy/font-strategy.md` §3).
...
## 실행 가능한 검증 — patch-guard MCP

위 핵심 불변식은 이 플러그인이 함께 배포하는 `patch-guard` MCP 서버(`mcp/patch_guard_mcp.py`)로 기계 판정한다. 서버는 표준 입출력 JSON-RPC로 붙고 언어에 의존하지 않으며, 각 판정은 `{"decision":"accept"|"reject"}`로 답한다. `reject`는 정상 판정 결과이지 전송 오류가 아니고, 잘못된 인자만 `invalid params`로 구분된다. 판정 대상 값이 확정되면 사람 판단 대신 이 도구로 경계를 강제한다.

| 불변식 | MCP 도구 | 판정 |
|--------|----------|------|
| 지원 원본 식별 | `verify_source` | 공급 바이트의 길이·SHA-256이 선언한 원본과 일치 |
| 변환 경계는 수정 전에 검증 | `verify_exact_roundtrip` | 선언한 무변경 왕복 경계를 바이트 단위로 재현 |
| 릴리스 준비·사람 승인 | `evaluate_readiness` | 개발 빌드는 미완료를 허용, 릴리스 후보는 완료와 승인 리비전 일치 요구 |
| 순수 원천에서 재생성 | `validate_product_graph` | 최종 산출물이 등록된 제품 그래프와 순수 원천만으로 재생성 |
| 실행 증거는 산출물에 묶임 | `require_runtime_pass` | 통과 런타임 증거가 게이트하는 정확한 산출물 해시와 일치 |
| Expected Write 사전 등록 | `apply_write_plan` | 겹침·보호 범위·기대 원본 불일치·추적되지 않은 diff를 실패 처리 |

기계어 쓰기는 ISA 검증기가 JSON 경계를 넘어올 수 없으므로 이 서버에서 항상 거부된다. 기계어 검산이 필요한 대상 프로젝트는 자신의 ISA 검증기를 프로세스 내부에 설치한 참고 코어를 쓰거나, 같은 판정을 다시 구현한다. 판정 로직과 반례의 원천은 `mcpads/create-kr-patch-template`의 `conformance/` 매니페스트와 Rust 참고 구현이며, 이 MCP 서버는 동일한 반례를 같은 결과로 판정한다.
