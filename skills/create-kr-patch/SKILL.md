...
- **Call a human for preference, not for difficulty.** A judgment that depends on taste, such as meaning, voice, naturalness, adaptation, or presentation, is a human's. A technical obstacle is yours: establish the missing observation through `references/strategy/debugging.md` §2.1, or report the check as not run. Volume, time, or an unavailable tool does not convert your work into a human decision.
