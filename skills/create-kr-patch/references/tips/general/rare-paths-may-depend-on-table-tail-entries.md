...
- **Failure context:** The product build omitted a special pointer at the end of a table and started the first text byte one byte early, overwriting that pointer.
- **Discriminating evidence:** The failing branch's actual load path was disassembled and compared with the source constants, pointer count, and first text address.
