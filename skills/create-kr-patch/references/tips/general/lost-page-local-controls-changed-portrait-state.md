...
- **Evidence:** Every source and translation page was checked for the same wait and output boundaries, and every original portrait control was confirmed before visible text. Value, order, duplication, and reset were restored per page, then compared against a control artifact with no portrait commands and the actual handler state.
