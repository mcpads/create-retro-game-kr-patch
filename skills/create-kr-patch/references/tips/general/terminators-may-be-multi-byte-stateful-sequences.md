...
- **Discriminating evidence:** Following the consumer separated `00 00` termination, `00 FF` button wait, and lone `FF` character data. The affected range was then re-extracted.
- **Established result:** Inspecting only the first byte of a control sequence caused the extraction error.
