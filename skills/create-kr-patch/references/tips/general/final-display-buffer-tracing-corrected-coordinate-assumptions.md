...
- **Discriminating evidence:** Changes were traced backward from the work buffer to the VRAM transfer, and each window's tile base was compared with the original initialization table. Separating the shared-window and normal-window base values fixed both price placement and dialogue order.
