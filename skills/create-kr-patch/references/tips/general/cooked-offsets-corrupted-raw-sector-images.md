...
- **Failure context:** A prior graphics offset used a pregap-free 2048-byte user-data ISO, while the product build input was a 2352-byte Mode 1 BIN with pregap. Mixing those coordinate systems targeted another sector.
- **Discriminating evidence:** A unique original byte sequence was located in extracted user data, pregap was calculated from CUE INDEX values, and the coordinate difference matched that calculation before only the affected sectors were rewritten.
