...
- **Discriminating evidence:** Consumer analysis showed that the control saved the address following the token before jumping to a shared block. The relocated path reconstructed that return target explicitly, and a source entry from the game verified the return path rather than only terminal-byte equality.
