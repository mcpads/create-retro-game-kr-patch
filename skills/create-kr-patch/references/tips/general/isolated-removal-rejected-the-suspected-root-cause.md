...
- **Discriminating evidence:** A diagnostic product artifact with only the suspected hook removed still failed. Following the DMA source then showed that the Korean font occupied WRAM used by the game.
- **Established result:** The actual cause was a WRAM collision, not the suspected hook.
- **Transfer limit:** Removing a suspect is a valid rejection experiment only when the removal does not change other relevant paths.
