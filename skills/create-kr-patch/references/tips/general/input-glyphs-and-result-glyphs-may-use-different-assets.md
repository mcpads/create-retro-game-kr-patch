...
- **Evidence:** A BNCG-only product artifact changed only the candidate grid. Changing both BNCG and the matching MES slot made the grid, editing and confirmation displays, and later name displays agree.
...
- **Transfer limit:** Trace candidate and result supply paths separately unless their equivalence is established. An NFTR that has no effect on one path remains a candidate elsewhere.
