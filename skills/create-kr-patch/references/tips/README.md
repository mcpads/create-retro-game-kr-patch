...
| Runtime screen evidence corrected extraction labels | Debugging / Text extraction | Human labels disagree with the data shown at runtime | Dreamcast | `references/tips/general/cases.md#runtime-screen-evidence-corrected-extraction-labels` |
| Zero-filled space is not proven free | Reinsertion and hooks / Initial survey | Code or data placed in an apparently unused region crashes on entry | Dreamcast / Saturn | `references/tips/general/cases.md#zero-filled-space-is-not-proven-free` |
...
| Equal raw bytes can have different consumer semantics | Translation / Text extraction | A token copied from another patch moves or renders differently | Game Gear / NES | `references/tips/general/cases.md#equal-raw-bytes-can-have-different-consumer-semantics` |
...
| Repeated glyph blocks may encode pre-shifted variants | Fonts and encoding / Runtime assets / Debugging | Similar data repeats per glyph and variable-width output clips or shifts | Mega Drive | `references/tips/general/cases.md#repeated-glyph-blocks-may-encode-pre-shifted-variants` |
...
| Asset expansion without metadata growth halted before title | Fonts and encoding / Debugging | Adding one asset entry causes a pre-title halt | Nintendo DS | `references/tips/general/cases.md#asset-expansion-without-metadata-growth-halted-before-title` |
...
| Standard-decoder rejection does not prove free code space | Fonts and encoding | An apparently unused lead byte is chosen from decoder statistics alone | PC-98 | `references/tips/general/cases.md#standard-decoder-rejection-does-not-prove-free-code-space` |
| Shared boundary logic let generator errors pass validation | Fonts and encoding / Build and verification | Generation and validation share the same mapping formula | PC-98 | `references/tips/general/cases.md#shared-boundary-logic-let-generator-errors-pass-validation` |
...
| Token width follows the consumer read unit | Text extraction | One-byte controls misalign a consumer that always reads two bytes | SNES | `references/tips/general/cases.md#token-width-follows-the-consumer-read-unit` |
...
| Rare paths may depend on table-tail entries | Build and verification | Only one branch corrupts immediately after entry | SNES | `references/tips/general/cases.md#rare-paths-may-depend-on-table-tail-entries` |
...
| Runtime samples can locate compressed source assets | Compression / Initial survey / Runtime assets | Live glyphs are known but the compressed source location is not | SNES | `references/tips/general/cases.md#runtime-samples-can-locate-compressed-source-assets` |
| Layout limits include physical writes and clear lifetime | Reinsertion and hooks / Runtime assets | Text fits logically but leaves residue or damages adjacent UI | SNES / NES / Mega Drive | `references/tips/general/cases.md#layout-limits-include-physical-writes-and-clear-lifetime` |
...
|---|---|---|---|---|
| NFTR tags and CMAP order follow on-disk consumer semantics | Fonts and encoding | Documented NFTR tags do not match raw bytes or a new CMAP entry is ignored | Nintendo DS | `references/tips/platforms/nds.md#nftr-tags-and-cmap-order-follow-on-disk-consumer-semantics` |
...
| NMI graphics hooks leaked across screen states | Reinsertion and hooks / Debugging | A graphics overwrite also runs on re-entry or neighboring screens | SNES | `references/tips/platforms/snes.md#nmi-graphics-hooks-leaked-across-screen-states` |
