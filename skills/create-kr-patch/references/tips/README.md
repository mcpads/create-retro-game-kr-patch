...
`references/tips/general.md`는 대상의 저장·소비 문법보다 증거의 출처·적용 시점, 실험 설계, 도구와 실제 소비자의 차이, 후보 공간의 실행 여부, 번역·검토 맥락이 선택 조건인 사례를 둔다. 해당 파일도 선독하지 않으며, ID의 플랫폼 접두사와 관측 범위를 유지한다. 같은 사건은 플랫폼 파일과 이 파일에 중복하지 않는다.

tip에 들어갈 수 있는 것은 관측 플랫폼·범위, 당시 자산·경로의 맥락, 구체 증상, 틀렸던 가정, 원인을 가른 실험, 확정된 수정 또는 기각 결론이 복원된 사례뿐이다. 어느 하나가 빠져 일반적인 명령문만 남으면 tip이 아니다. 출처 범위를 복원할 수 없는 조언, 공개 검색으로 재구성 가능한 배경지식, strategy의 판단 기준을 되풀이한 문장은 두지 않는다.
