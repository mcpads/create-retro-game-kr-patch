...
| Zero-length entries may point to runtime text | Reinsertion and hooks / Runtime assets | Clearing a zero-length entry makes the game parse unrelated bytes as text | Game Gear | `references/tips/general/cases.md#zero-length-entries-may-point-to-runtime-text` |
...
| Expanded assets may require a decompressed-size update | Compression / Reinsertion and hooks / Runtime assets | New glyphs exist in the expanded file but live font memory stays zero | Saturn | `references/tips/general/cases.md#expanded-assets-may-require-a-decompressed-size-update` |
...
| Address reads do not prove semantic consumption | Debugging / Compression / Runtime assets | A read breakpoint on the asset fires, but changing it has no visible effect | Saturn | `references/tips/general/cases.md#address-reads-do-not-prove-semantic-consumption` |
...
| Later original writes overwrote replacements | Runtime assets / Reinsertion and hooks | A patched logo or subtitle appears briefly and then disappears | SNES / PC Engine CD | `references/tips/general/cases.md#later-original-writes-overwrote-replacements` |
...
| Font parsing success does not prove usable glyphs | Fonts and encoding | A font file parses and builds, but text areas stay blank | SNES | `references/tips/general/cases.md#font-parsing-success-does-not-prove-usable-glyphs` |
...
| Asset reachability does not prove readable text | PoC / Runtime assets / Fonts and encoding | Patched bytes verify against VRAM but the screen text is still unreadable | SNES | `references/tips/general/cases.md#asset-reachability-does-not-prove-readable-text` |
...
| Reloaded hooks mixed instruction-cache line states | Reinsertion and hooks / Runtime assets / Debugging | A reloaded hook behaves as if only part of the patch applied | PlayStation | `references/tips/platforms/ps1.md#reloaded-hooks-mixed-instruction-cache-line-states` |
| Composite glyph layout changed tilemap arithmetic | Fonts and encoding / Runtime assets | Glyph tiles reach the intended address but the screen keeps reading other tiles | SNES | `references/tips/platforms/snes.md#composite-glyph-layout-changed-tilemap-arithmetic` |
