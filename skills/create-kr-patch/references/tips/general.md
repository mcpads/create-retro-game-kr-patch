...
- **전이 한계:** 빈 패딩·정적 참조 0건·패치 간 비중첩은 후보 신호일 뿐이다. 원본 코드·데이터의 사용 여부는 별도 증거로 판정한다.
...
- **전이 한계:** save state는 이후 장면을 재현할 수 있지만, 생략된 적재·업로드·캐시 갱신의 부재를 증명하지 않는다.
...
- **전이 한계:** 다른 프로젝트에서는 특정 문자 검색만으로 stale 번역을 판정하지 않고 원문 식별값과 디코드 변화의 영향 범위를 비교한다.
...
- **전이 한계:** 표현 축소는 공급 확장 가능성을 판정한 뒤, 현재 코퍼스의 실제 절감량과 의미 손실을 승인해 선택한다.
...
- **전이 한계:** 다른 장면은 대상 영역의 마지막 writer를 다시 추적한다.
...
- **전이 한계:** 라이브러리·font table·대상 codepoint·raster 경로가 바뀌면 대표 글리프 프로브를 반복한다.
