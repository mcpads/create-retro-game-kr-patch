...
- **Discriminating evidence:** Tracing the tilemap writer showed that horizontal indexing converted character code `N` to `N×2` and `N×2+1`. One visible glyph was a 2x2 arrangement of that left-right pair; address-only experiments had omitted this transform.
