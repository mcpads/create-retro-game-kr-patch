...
The 65816 M/X state changes immediate-instruction length as well as register width. A hook or moved source code must preserve source instruction boundaries and effects with entry M/X state, bank, direct page, stack, and live flags.
...
Growing the ROM file does not make a new bank readable by the CPU, hardware, or loader. Place assets in expanded space only when mapper, header, checksum, save-memory and mirror conflicts, and the actual distribution target can represent it.
...
VRAM writes and DMA must fit the game's blanking, NMI queue, and transfer order. If a patch appears briefly and disappears, identify every writer, the last writer, reload after screen transition and re-entry, and other consumers of the staging region.
