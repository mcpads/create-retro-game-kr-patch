...
원본이 쓰는 코드도 선언한 범위를 전량 현지화한 뒤에는 재배정 후보가 된다. 다만 비텍스트 그래픽이 같은 슬롯을 소비하거나, 미번역 범위나 `references/conventions/translation-artifacts.md` §5의 승인 예외가 그 문자를 계속 요구하거나, 글리프가 BIOS·폰트 ROM처럼 수정 대상 밖에서 공급되면 회수 대상이 아니다.
...
4. 비글리프 자원, 미사용 슬롯과 선언한 범위를 전량 현지화해 더는 소비되지 않는 원본 글리프를 회수하려면 모든 참조자와 상태를 배제한 뒤에만 공급으로 계산하고, 전체 레퍼토리와 활성 작업 집합 가운데 어느 예산이 늘어나는지 따로 판정한다.
