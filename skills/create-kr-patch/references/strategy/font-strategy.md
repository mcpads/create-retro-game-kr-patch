...
폰트 전략은 특정 폰트 제품, 변환 도구, 셀 크기나 캐시 구현을 고르는 문서가 아니다. 대상 리비전의 문자 소비자가 요구하는 코드→글리프 계약, 전체 저장 레퍼토리, 상태별 활성 작업 집합, 표현 형식과 검증 게이트를 정한다. 실제 병목이 드러난 축부터 판정하되 서로 다른 예산을 하나의 숫자로 합치지 않는다.
...
1. 배포 대상 문자열 모집단과 파서가 도달 가능한 후보 영역을 닫았다.
2. 원본 문자·외자·제어 토큰·종료자와 예약값을 구분했다.
3. 해당 값을 소비하는 모든 디코더·조회·우회 경로가 새 계약을 이해한다.
...
- 전체 레퍼토리를 수용할 수 없고 공급 확장도 성립하지 않으며, 자모 합성·배치·클리어의 추가 소비 계약을 검증할 수 있을 때 조합형을 후보로 둔다.
...
5. 전체 레퍼토리가 부족하면 정확한 병목을 닫고 공급 확장 가능성을 판정한다. 번역 어휘 축소나 문자 대체는 의미·말투 손실을 사람이 승인한 경우에만 사용한다.
...
- 같은 고정 입력과 변환 계약에서 같은 게임 데이터를 재현한다.
- 실제 소비자가 요구하는 배열·비트 깊이·팔레트·서브타일 순서를 만족한다.

원본 표현을 재사용할 수 있으면 변경 표면이 작은 후보가 되지만 의무는 아니다. 셀, 비트 깊이, 배열이나 글리프당 크기를 바꾸면 그 값을 소비하는 주소 계산, 전송량, 인덱스, 클리어와 레이아웃 계약을 함께 갱신하고 검증해야 한다. 저장 형식과 활성 형식이 다르면 변환 경계와 버퍼 수명도 별도 계약으로 둔다.

## 5. 복수 렌더 경로

본문·UI·이름 입력·그래픽 경로가 다른 공급자나 표현 계약을 쓰면 경로별 코드표, 글리프 원천, 셀과 예산을 따로 판정한다. 한 경로의 PoC를 전체 게임으로 확대하지 않는다.

여러 경로가 같은 표현 파라미터를 공유한다면 하나의 명시 입력을 정본으로 삼는다. 파라미터가 실제로 달라 서로 덮어쓰거나 빌드와 검토 결과가 어긋나는 부채가 생겼을 때만 타깃별 프로필을 분리한다. 프로필의 저장 형식이나 덮어쓰기 수단은 프로젝트가 정하되, 알 수 없는 타깃·값·누락을 묵인하지 않는다.
...
- 배포 범위의 모든 문자가 한 코드·매핑 정본에 배정되고 미할당이 0이다.
