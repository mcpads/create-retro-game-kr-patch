...
Do not set a global font family, cell size, bit depth, or transform. Unless the approved visual target calls for original letterforms, begin by evaluating established fonts whose provenance, distribution terms, and required character coverage are known. Compare candidates by sending the same strings through the target game's transform. Survey whether size, baseline, rasterization, or margin adjustments can meet the target and expose their cost and visual effect before concluding that new letterforms are necessary.

Local additions or corrections are a low-cost choice when an established font otherwise meets the approved target. Do not infer a need for a new set merely from PoC convenience. A complete custom font remains a human product and visual-design choice when the approved target justifies its cost. When limited additions cannot cover the selected scope, treat it as one technical option and present any material cost, quality, or scope tradeoff under `references/conventions/project-records.md` §1.1.
