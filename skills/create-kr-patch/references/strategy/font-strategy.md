...
폰트 전략은 특정 폰트 제품, 변환 도구, 셀 크기나 캐시 구현을 고르는 문서가 아니다. 대상 리비전의 문자 소비자가 요구하는 코드→글리프 대응, 전체 저장 레퍼토리, 상태별 활성 작업 집합, 표현 형식과 검증 게이트를 정한다. 실제 병목이 드러난 부분부터 판정하되 서로 다른 예산을 하나의 숫자로 합치지 않는다.
