...
다음 변경이 하나라도 있으면 이 문서의 연결 검증을 적용한다.
...
위 조건에 해당하지 않고, 같은 지원 리비전·동일 소비 동작의 기존 증거가 변경 전후 체인이 같음을 직접 보장하는 단순 교체에는 별도 연결 검증이 필요하지 않다. 다른 리비전·로더·상주 방식이나 범위가 더 좁은 과거 증거로 생략하지 않는다.
