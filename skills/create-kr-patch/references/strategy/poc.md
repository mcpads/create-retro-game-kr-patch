...
Use a PoC to resolve uncertainty that can change distribution feasibility before dependent implementation.
...
Choose a unit that connects the complete extraction-to-consumption boundary and exercises the applicable constraint that is hardest to satisfy: length, control tokens, pointers, compression, padding, or another boundary whose failure would invalidate scale-up. A short string that bypasses that condition is not representative. If the decisive constraint remains unresolved, establish it before claiming a representative unit or state the narrower supported claim.
...
A temporary code mapping supports only the representative claim, not collision freedom across the distribution. Apply `references/strategy/font-strategy.md` §2.1 to the distribution mapping decision.
