...
각 항목은 독립된 증명 범위를 가진다. 가시성은 실제 데이터 경로를 거친 한글 픽셀 도달을, 대표 E2E는 선언한 단위의 추출→소비 연결을 증명한다. 글리프 예산·인코딩 공간·재배치·압축과 다른 렌더 경로·자산 종류는 해당 항목의 별도 증거로 판정한다.
...
대표 단위는 대사 블록, 메뉴 화면, 페이지처럼 실제 빌드가 처리하는 최소 단위다. 길이·제어 토큰·포인터·압축·패딩 중 목표 범위에 실제로 존재하는 제약을 포함해야 하며, 그 제약을 우회한 짧은 문자열은 대표 증거가 아니다. 표시 예산이 설계를 바꿀 수 있으면 가장 제한적인 창·렌더 경로나 원본 사용 범위 부근의 단위를 우선한다.
...
PoC 결과를 본 구현에 채택하면 확정된 규칙과 입력을 주 빌드 경로에 연결하고, 그 시점까지 채택한 다른 변경과 함께 불변 원본에서 개발 빌드를 다시 만든다. 전체 번역이 끝나지 않았으면 `references/conventions/translation-artifacts.md` §5의 개발·PoC 입력 정책으로 원문을 보존할 수 있으나, 이를 이유로 글리프·인코딩·재삽입·코드 변경을 서로 분리된 실험물로만 남기지 않는다. 각 구성요소의 독립 성공은 `references/strategy/build-and-verify.md` §1의 통합 빌드를 통과할 때까지 전체 구현의 성공이 아니다.
