...
도달 수단은 고정 순서가 아니라, 판정에 필요한 선행 상태를 보존하는 후보 중 재현 비용이 가장 낮은 것을 고른다. 정상 플레이 경로, 호환성을 확인한 저장 상태·입력 기록, 게임 내부의 진입 수단, 검증된 치트·상태값 변경, 확인된 상태 전이 루틴 호출이 후보가 될 수 있다. 인위적 개입이 적다는 이유만으로 오래 걸리거나 비결정적인 플레이를 반복하지 않고, 빠르다는 이유만으로 판정에 필요한 선행 상태를 건너뛰지 않는다.
...
개입으로 만든 상태는 보존한 선행 조건과 목표 상태 이후의 소비만 증명한다. 개입이 건너뛴 호출자 선택, 획득 조건, 분기·이벤트 상태, 저장·불러오기나 다른 상태 전이는 증명하지 않는다. 이 경로가 판정 대상이면 정상 플레이 경로나 확인된 호출자 경로로 별도 검증한다. 조사·검증에만 쓴 개입은 패치 내용이나 반복 빌드의 입력에 포함하지 않는다. 개입 기록이 보존할 의미는 `references/conventions/project-records.md`의 공통 기록 규칙을 따른다.
...
화면, 로그, 크래시 지점, 마지막 쓰기 주체는 사슬의 끝일 수 있다. 저장, 변환, 적재, 상주, 소비 가운데 기대값과 처음 달라지는 경계를 찾고, 수정이 그 경로를 끊는 이유를 증명한다.
...
자동 증거만으로 판정할 수 없어 사람 관측이 필요하면, 가능한 결과가 경쟁 가설과 다음 행동을 구분하도록 요청한다. 응답이 구분력을 만들지 못하면 결론을 확정하지 않고 더 작은 관측으로 좁힌다. 사람이 본 패턴은 결정적이고 계산 가능한 검사로 바꿀 수 있을 때만 자동화한다.
...
- 호출되지 않음, 미사용 공간, 상태 소유자는 선언한 분모를 전수 확인한 경우에만 확정한다.
