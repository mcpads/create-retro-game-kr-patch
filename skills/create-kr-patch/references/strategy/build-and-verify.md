...
이 문서는 빌드 입력, 배포 산출물, 정적 검증, 실행 검증의 통과 조건을 다룬다. 플랫폼별 체크섬·섹터·실행 환경 사실은 `references/platforms/`가 소유한다. 기록이 보존할 의미와 검증 규칙은 `references/conventions/`를 따르고, 구체 파일·명령·직렬화는 대상 프로젝트가 정한다.
