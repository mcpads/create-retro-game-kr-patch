...
- Player-facing release material identifies the supported source revisions and representations, the verified application procedure and prerequisites, every known limitation or scope exclusion that can affect play, and save-data compatibility and any required migration when the patch changes persistent data. The declared source profile under `references/conventions/project-conventions.md` §6.2 owns supported-source identity. The verified distribution and application path under `references/strategy/build-and-verify.md` §2 owns the procedure and prerequisites. Current QA, decision, and issue records under `references/conventions/project-records.md` §1.1 and §7 own limitations, exclusions, and save-data compatibility or migration. Player-facing material presents these facts; it does not redefine their technical or product authority.
