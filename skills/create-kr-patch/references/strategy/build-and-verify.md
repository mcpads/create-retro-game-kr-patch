...
- Player-facing release material identifies the supported source revisions and representations, the verified application procedure and prerequisites, every known limitation or scope exclusion that can affect play, and save-data compatibility and any required migration when the patch changes persistent data. It derives these facts from the declared source profile under `references/conventions/project-conventions.md` §6.2 and the current decision and issue records under `references/conventions/project-records.md` §1.1 and §7.3, and does not become a separate authority for them.
