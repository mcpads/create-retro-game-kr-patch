...
초기 조사는 게임 전체를 해명하기보다 **다음 구현을 승인하거나 보류할 수 있을 만큼 불확실성을 줄인다**. 조사 순서와 도구는 현재 판단을 가장 싸게 바꾸는 증거에 맞춰 선택한다. 플랫폼 고유 사실은 `references/platforms/`에서 확인하고, 조사 결론의 증거·판정·다음 행동은 `references/conventions/project-records.md` §3에 따라 구분한다.
...
새 인코딩 공간, 글리프 수용량, 이름 입력, 재배치, 압축 재구현처럼 기본 가시성 PoC와 독립된 위험은 조건부 검증으로 분리한다. PoC의 증명 범위와 분기 기준은 `references/strategy/poc.md`가 소유한다. 그래픽에 구운 텍스트는 `references/strategy/graphics-text.md`로 분기한다.
...
다음 조사 대상은 **현재 결정을 가장 싸게 가르는 증거 앵커**에서 고른다. 앵커는 알려진 화면 출력, 원시 바이트, 포인터나 테이블, 글리프 원천, 실제 읽기·쓰기 코드처럼 저장과 소비 중 어느 쪽에 있어도 된다. 앵커에서 양방향으로 확장하되, 새 조사가 다음 중 하나를 바꾸는지 먼저 묻는다.
...
후보 설계가 `references/strategy/runtime-assets.md` §1의 발동 조건에 해당하면 저장 위치만으로 완료하지 않는다. 로더, 주소 계산, 버퍼, 전송, 캐시·재적재, 최종 소비의 연결 검증은 그 문서가 소유한다.

### 2.5 초기 분량 조사

현지화 대상의 저장·소비 경계가 드러나면 `references/strategy/text-extraction.md` §1.5에 따라 대상 종류별 모집단과 분량을 조기에 조사한다. 전체 범위를 열거하는 비용이 크고 PoC가 설계를 먼저 뒤집을 수 있다면 확인된 하한과 미확정 범위를 기록한 뒤 대표 PoC를 먼저 수행할 수 있다.

초기 조사나 PoC에서 추가 가정 없이 전체 범위를 열거할 수 있는 파서·테이블·참조 경계를 확인하면 표본에서 멈추지 않고 분량 조사로 돌아간다. 후보량을 번역 작업량으로 확정하지 않으며, 전체 배포 범위의 번역 확대를 승인할 수 있는지는 `references/strategy/text-extraction.md` §1.5가 판정한다.
...
번역 길이·글리프 수·재배치 결과에 따라 달라지는 끝 주소·분기 거리·파일 크기·체크섬은 입력에서 파생한다. 여러 리비전, 런타임 생성 데이터, 아직 범위가 확정되지 않은 호출 경로처럼 모집단이 미확정인 경우에는 고정 목록으로 가장하지 않고 파싱·탐지와 미확정 실패 경로를 유지한다. 휴리스틱 결과가 고정 사양과 다르면 자동 보정하지 않고 리비전·분모·거짓양성을 다시 판정한다.
...
산출물은 현재 결론을 담는 아키텍처 기록, 초기 분량을 포함한 텍스트·자산 맵, go/no-go 위험표다. 분석 일지는 시간순 증거로 남길 수 있지만 현재 결론을 대신하지 않는다. 기록해야 할 의미와 관계는 `references/conventions/project-records.md` §3을 따른다.

초기 조사 결과는 아직 판정되지 않은 다음 질문 또는 승인된 구현 경계로 돌려준다. PoC 발동 조건이 없거나 동등 증거가 있으면 반복 실험을 만들지 않는다. 검증이 실패하면 전체 조사를 반복하는 대신 실패한 경계 하나를 다음 조사 대상으로 삼는다. 플랫폼별로 어떤 경계가 존재하는지는 필요한 경우에만 해당 `references/platforms/<platform>.md`를 읽고 대상 게임에서 다시 확인한다.
