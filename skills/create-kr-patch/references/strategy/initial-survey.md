...
초기 조사는 게임 전체를 해명하기보다 **선언한 완성 범위의 실현 가능성과 다음 구현을 판정할 만큼 불확실성을 줄인다**. 실패하면 완성이 불가능하거나 설계를 크게 바꾸는 조건을 먼저 찾고, 그 조건을 판정할 수 있는 증거 가운데 비용이 낮은 것을 선택한다. 플랫폼 고유 사실은 `references/platforms/`에서 확인하고, 조사 결론의 증거·판정·다음 행동은 `references/conventions/project-records.md` §3에 따라 구분한다.
...
다음 조사 대상은 **완성 여부를 좌우하는 미확정 조건**에서 고른다. 우회안이 없는 용량·표현·소비 경계처럼 하나의 실패가 선언한 범위를 막거나 기본 설계를 폐기하게 만드는 조건이 우선이다. 같은 조건을 판정할 수 있는 후보가 여럿이면 필요한 선행 상태와 증명 범위를 보존하면서 비용이 낮은 증거 앵커를 고른다. 쉬운 부분의 성공만 보여 주고 해당 조건을 그대로 남기는 증거는 우선하지 않는다.

앵커는 알려진 화면 출력, 원시 바이트, 포인터나 테이블, 글리프 원천, 실제 읽기·쓰기 코드처럼 저장과 소비 중 어느 쪽에 있어도 된다. 앵커에서 양방향으로 확장하되, 새 조사가 다음 중 하나를 바꾸는지 먼저 묻는다.
...
후보 설계가 `references/strategy/runtime-assets.md` §1의 발동 조건에 해당하면 로더, 주소 계산, 버퍼, 전송, 캐시·재적재와 최종 소비의 연결을 같은 문서 §2에 따라 검증한다.
