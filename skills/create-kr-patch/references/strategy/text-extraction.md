...
대표 단위의 기술 PoC와 배포 범위의 분량 판정은 다르다. 정적으로 닫힌 파일·자산 목록이나 초기 조사·PoC에서 확인한 파서·테이블·참조 경계가 추가 가정 없이 선언한 범위를 열거할 수 있으면 표본에서 멈추지 않고 그 시점에 전체 범위를 열거한다. 새 화면이나 파일을 발견할 때마다 수동으로 항목을 보태는 방식을 전체 분모의 기본 수립 방법으로 삼지 않는다.

분량은 현지화 대상 종류와 소비자 범위별로 집계하며 다음을 구분한다.

- 전수 열거한 모집단과 그 안의 해결·제외·미확정 항목 수
- 확인된 구조에서 얻은 확정값, 아직 열린 범위가 있는 하한, 근거가 제한된 추정치
- 열거하지 못한 컨테이너·실행 경로, 열린 이유와 닫기 위한 다음 증거 또는 종료 조건
- 엔트리 수, 원문 문자·코드 단위와 원시 바이트량, `references/strategy/graphics-text.md` §1에서 확정한 그래픽 텍스트 블록 수 가운데 번역 작업량·저장·표현 설계를 바꾸는 측정값과 단위
...
기술 PoC와 분모가 닫힌 국소 범위의 번역은 이 판정 전에도 수행할 수 있다. 번역 작업을 전체 배포 범위로 확대하기 전에는 선언한 대상 종류별로 모집단을 전수 열거할 수 있는지 판정하고, 가능한 범위는 전수 열거한다.

구조상 전수 열거할 수 없으면 확인된 하한·열린 범위·종료 조건을 명시한다. 남은 분량이 작업량 판정이나 기술 설계 선택을 바꿀 수 있는 미확정성으로 남아 있으면 전체 범위로 확대하지 않는다. 모집단이 열려 있는 동안 정확한 전체 작업량·완료율·전체 코퍼스 수요를 주장하지 않는다.
