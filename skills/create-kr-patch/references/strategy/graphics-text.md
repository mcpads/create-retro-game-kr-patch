...
A text-free background must be an adopted comparison reference against which protected regions can be compared. If it cannot be regenerated deterministically, freeze the adopted result and verify identity against it. The asset is incomplete when the boundary between source text and background is unresolved, or when a clean environment cannot reproduce and verify the result.
...
- The product build reproduces protected-pixel, palette, container, and unchanged-round-trip checks.
