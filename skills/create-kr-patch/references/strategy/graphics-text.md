...
- 번역 영역이 승인된 조판·시각 기준을 따르고 실제 배경·팔레트에서 잘림 없이 판독되며, 대상에 존재하는 상태별 시각 구분이 유지된다.
- 모든 대상 상태·변형이 실제 소비 경로에서 확인됐다.
- 변경된 자산의 저장→적재→선택→표시 연결이 입증됐다.
- 수정하지 않은 대표 소비 경로가 기준 동작을 유지한다.

분모가 미확정이거나, 배경·보호 영역을 검증할 수 없거나, 공유 자산의 소비 경로를 분리하지 못했거나, 실제 소비자를 연결하지 못했거나, 외부 대체 경로에서만 보이면 **미완료**다. 파일 모집단에 없는 표시가 남아 있어도 누락된 저장·합성 계층이 있다는 뜻이므로 미완료다. 미완료 항목은 배포 적격 범위에서 제외하거나 원인을 해소하기 전까지 완료로 승격하지 않는다.
