...
2. Product build path and code boundary
3. Interfaces and exchange data
4. Reproducibility requirements for external components
5. Tooling build, analysis, product build, test, and verification responsibilities
6. Source-asset handling

## 1. Ownership and authoritative sources

Before creating or extending a repository, inspect its current product build path, tests, documents, and asset flow. Map only the responsibilities the work uses to the repository's existing vocabulary and owners. Typical responsibilities include:

- source inputs to the reproducible product build;
...
Use status terms by meaning and authority. **Established** means evidence supports a fact, constraint, or interpretation within a stated scope. **Adopted** means a result, decision, specification, design, or contract is active in the project baseline; adoption records current use, not who had authority to choose it. **Approved** means a human has made a judgment that requires human authority. **Existing** means only that something is already present. One status does not imply another.
...
Investigation experiments, heuristic discovery, and one-off transforms stay outside the primary product build. Adopt a conclusion as a specification with its evidence scope, applicability, and reassessment conditions. Adopt its producing procedure separately only when it has a stable deterministic output or enforces a recurring product build gate; then declare its inputs, outputs, version, and failure propagation. Reopen discovery only on an applicability failure, reassessment condition, or counterevidence. Give adopted production code a role-based name and isolate or retire competing probe paths.
...
Throughout this skill, a **product build** constructs the patched game and any distribution output. A **product artifact** is its exact playable result; when play requires several files or images, the artifact is the identified set. A **distribution output** is the patch or package that reproduces that artifact. Qualify **candidate** by the object or purpose under consideration, such as a pointer candidate, wording candidate, pre-release review candidate, or release candidate; the word alone does not identify an artifact. Compiling or packaging the patcher, analyzer, or supporting code is a **tooling build**; it does not run the product build or establish a claim about the product artifact or distribution output.

Extraction, transformation, reinsertion, and patch generation must rerun from source through one documented primary product build path. That path starts from the adopted inputs and specifications under §1; it reconstructs the product, not the investigation history that established them. A primary product build is one execution of that path. External components may participate, but the path must fix their adopted version, input, output, options, failure conditions, and output verification. A secondary path must not modify the same product artifact or distribution output independently or inherit an untracked manual result.
...
An existing project may retain several components or languages when they already form an equivalent reproducible path. They need not become one executable if responsibility and failure remain traceable in one product build graph.
...
When an external program, library, or service affects output from the primary product build, bind its source and version to declared inputs, options, outputs, failure propagation, and applicability. Retain target-specific round-trip or cross-validation evidence, independent output verification in the primary product build, and the license basis for bundling or redistribution. Preserve an existing mechanism that already satisfies these requirements.
...
The product build must fail when:
...
### 5.3 Tooling build, analysis, product build, test, and verification responsibilities

Classify work by the claim it establishes and the inputs that can change that claim, not by the command that happens to run it or whether it is automated. One executable or command may orchestrate several roles, but their results and claim boundaries remain distinct. Running current source or product inputs through an analyzer or verifier remains analysis or artifact verification even when a test runner invokes it.

| Responsibility | Use it when | It establishes | Boundary |
|---|---|---|---|
| Tooling build | The patcher, analyzer, verifier, or supporting code must be compiled or packaged. | The required tooling can be produced under the declared toolchain. | A tooling build does not run analysis, construct or verify the current product artifact or distribution output, or establish runtime behavior. |
| Analysis | The structure, explanation, applicability, or method is still uncertain. | Candidate evidence and, when adopted, a scoped specification with reassessment conditions. | Keep heuristic search, probes, and candidate selection outside recurring product builds and tests. A test may protect a deterministic contract adopted from analysis; it does not rediscover the contract or treat a current analysis result as an expected value. |
| Product build | The result depends on the current declared source image, translation, assets, configuration, or adopted specifications. | Derived product values, rejection of invalid current inputs, and the exact product artifact and distribution output to verify. | Do not pin mutable product results in tests. Calling an investigative procedure from the product build does not make it a product build component unless it has been adopted under §1 with a stable deterministic output or recurring gate. |
| Test | A stable interface, data format, deterministic invariant, minimal fixture, or reproduced failure is exercised under controlled inputs. | Continued conformance of an implementation or boundary to that contract. | A passing test neither analyzes the current source, selects current product inputs, constructs or verifies the current product artifact or distribution output, nor proves live consumption. |
| Artifact verification | A static claim concerns one exact source, product artifact, or distribution output and its declared profile, inputs, write plan, or container structure. | Static integrity and applicability of that object under the checked conditions. | Running a verifier from a product build or test does not turn artifact evidence into compilation or test evidence, or a static result into runtime proof. |
| Runtime verification | The claim depends on the game, loader, renderer, state transition, persistence path, or target environment consuming an exact artifact. | Observed behavior on the declared route and state, bound to that artifact and environment. | Automation does not turn runtime evidence into a product build gate or ordinary test. Runtime samples do not establish static population coverage unless the remaining members are tied to the same proven consumer path. |

Use **test** only for the Test responsibility above. A hypothesis-discriminating intervention remains an investigation experiment even when automated. Inspection of an exact artifact remains artifact verification, and observation of game consumption remains runtime verification, regardless of the command or runner used.

A test exercises an adopted contract or reproduced failure under controlled inputs and asserts observable boundary behavior. Its scope follows that behavior; do not replace an execution-dependent contract with static inspection. A checker test is meaningful when controlled valid input is accepted and a relevant defect is rejected. Tests that only confirm another check exists, was invoked, produced a report, or agreed with shared logic do not strengthen the product claim; neither do test counts, assertion counts, validation layers, or command success. Consolidate or remove tests that cannot distinguish conformance from a relevant contract defect.

A value expected to change under valid edits to declared translation, assets, or configuration is a result of the product build, not a stable test expectation. Let the product build derive and report current counts, addresses, displacements, sizes, and checksums. Tests use controlled fixtures to protect the invariant relating those values to source identity, capacity, layout, or artifact consistency. Exact values for a fixed source profile or frozen product artifact belong to artifact verification; pin a value in a test only when a minimal fixture or reproduced failure fixes it, and state the contract and change condition.

When producers, analyzers, generators, verifiers, or reports encode the same established fact or adopted specification in different representations, derive them from the authoritative source or verify their conversion or conformance contract. Compilation or testing each component against its own literals does not establish agreement.

Integration tests exercise stable interfaces between components with controlled fixtures. This includes product build and artifact verification boundaries when those interfaces are the subject of the test. Tests do not own current product inputs or replace the primary product build and artifact verification of the cumulative product artifact. `references/strategy/build-and-verify.md` owns current product criteria and their link to separate runtime evidence; tests may exercise acceptance and rejection with controlled inputs, but the product build enforces them for current product inputs.

When source media is unavailable, report source-dependent checks as explicitly not run and continue schema validation, translation checks, and unit tests that do not require it. Never report a partial run as complete success. Use `references/strategy/build-and-verify.md` for runtime criteria and `references/conventions/project-records.md` for evidence semantics.
...
Before interpreting or modifying content as a supported product build input, determine its revision and region, compute its size and a strong content hash, and match them to a declared source profile. An unknown source may be examined under an explicit survey-only baseline to establish its identity and structure, but a supported product build must reject it until the applicable profile and coordinates are established. Do not promote survey findings from that input into another profile silently. Maintain expected identity in one code location or validated manifest. When supporting several sources, separate coordinate and format profiles and record the selected profile in output.

Regenerate source-derived product build inputs such as palettes, tile bases, glyph originals, and baked graphics from declared source when possible. Prior-patch extracts and manual working files may provide comparison or bootstrap evidence but must not enter the product build without independent verification against declared source. When an irreproducible local derivative is unavoidable, document its limit, identity hash, necessary rights, and verification procedure, and fail the product build when it is absent.
