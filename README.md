...
현재 버전은 `main` 0.7.0, `next` 1.0.0-alpha.2이며, 고정된 프리릴리스 스냅샷은 `v1.0.0-alpha.2` 태그다. `next`는 이후 최신 개발 상태를 계속 따라간다. 두 채널은 같은 플러그인 ID를 사용하므로 동시에 설치하지 않고, 기존 marketplace를 원하는 Git ref로 교체한다.
